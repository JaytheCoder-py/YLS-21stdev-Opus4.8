...
- `web/app/page.tsx` stacks four sections in the "Halide" dark/silver/orange aesthetic: the hero (`web/components/ui/halide-landing.tsx`), a "Company Profile" band (`web/components/ui/company-profile.tsx`), a "Global Network" band (`web/components/ui/global-network.tsx`) built on an animated `dotted-map` world map (`web/components/ui/map.tsx`), and an "Upcoming Global Exhibitions" band (`web/components/ui/exhibitions.tsx`). The content sections share a section shell (`[ bracketed ]` Geist-Mono eyebrow → Syncopate UPPERCASE `h2` → orange rule) and reference the Halide tokens via `var(--bg|--silver|--accent, <fallback>)` so they render correctly even without the hero mounted. Several placeholders still need real data before publishing: the map's city routes are **illustrative**, not verified YLS geography; `company-profile.tsx` keeps external stock imagery and a non-functional "Company Overview" video card; and `exhibitions.tsx` holds placeholder event/date/booth data — swap these for `assets/` imagery and `company.md`-sourced facts. This Halide aesthetic is **not** the BMW-M-derived design language in `design.md` — reconcile before treating it as the YLS site's real design.
- `web/app/layout.tsx` wraps the app in a next-themes `ThemeProvider` (`web/components/theme-provider.tsx`) set to **dark, `enableSystem={false}`** — the site is effectively dark-only, so Tailwind `dark:` variants and `useTheme()` resolve to dark app-wide. Theme-dependent rendering needs a mounted guard to avoid an SSR hydration mismatch (see `map.tsx`).
